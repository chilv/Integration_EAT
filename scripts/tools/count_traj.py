...
def count_traj(args):
    ...
    for pkl in tqdm(dataset_path_list):
        ...
        body_vec.append(np.concatenate(bodies, axis=0).transpose()[int(pkl.split('.')[0].split('_')[-1])])  #仅保留有折损的body列
    ...
    vp = ax.violinplot(dataset = body_vec, positions = range(len(body_vec)), #widths=0.5,
                   showmeans=True, showmedians=False, showextrema=False)         
    
    # styling:
    for body in vp['bodies']:
        body.set_alpha(0.9)
    ax.set(xlim=(-0.5, 11.5), xticks=np.arange(0, 12),
        ylim=(-0.05, 1.05), yticks=np.arange(0, 1.05, 0.05))

    plt.savefig(os.path.join("./Integration_EAT/scripts/tools/traj_analyse/", args["dataset"]), dpi=150)
    print("fig saved")
    return 
