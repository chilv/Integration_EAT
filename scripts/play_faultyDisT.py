...
def play(args, faulty_tag = -1, flawed_rate = 1):
    ...
    model_path = os.path.join(parentdir, "EAT_runs/DistT_IPPO_01/")
    ...
    model.load_state_dict(torch.load(
        os.path.join(model_path,"model9.pt"), map_location = "cuda:0"
    ))
    model.eval()
    #====================================================================================
    ...
    with torch.no_grad():
        if True:
            ...
            for t in range(max_test_ep_len):
                ...
                running_state, _, running_reward, done, infos = env.step(act, flawed_joint = [faulty_tag], flawed_rate = flawed_rate) #if t > max_test_ep_len/8 else env.step(act, [-1])
