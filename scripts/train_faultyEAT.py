...
def partial_traj(dataset_path_list, context_len=20, rtg_scale=1000, body_dim=12):
    ...
    '''
    big_list = []
    # for pkl in tqdm(dataset_path_list[20*slices:20*slices+20]):#轨迹太长适当拆解,每20个一组
    for pkl in tqdm(dataset_path_list):  
        with open(pkl, 'rb') as f:
            thelist = pickle.load(f)
            #----------------------------------
            #试图记录轨迹的均值和标准差以便后续测试时读取
...
def train(args):
    ...
    datafile, i_magic_list, eval_body_vec, eval_env = get_dataset_config("continue4000_faulty")
    ...
    # print("D4RLTrajectoryDataset complete!")
