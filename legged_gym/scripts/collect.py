...
def collect(args):
    ...
    obs_buffer = torch.zeros(
        size=(env.num_envs, int(env.max_episode_length) + 1, env.num_obs - privileged_dim), device=device)
    ...
    for it in range(10000):
        ...
        with torch.inference_mode():
            ...
            obs_buffer[range(env.num_envs), episode_length_before_reset, :] = obs[:, :-privileged_dim]
