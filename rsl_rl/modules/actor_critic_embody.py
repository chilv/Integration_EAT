...
class ActorCritic_Embody(nn.Module):
    is_recurrent = False
    def __init__(self,  body_dim,
                        num_actor_obs,
                        num_critic_obs,
                        num_actions,
        ...
        super(ActorCritic_Embody, self).__init__()
        
        activation = get_activation(activation)
        print(num_actor_obs, body_dim)
        mlp_input_dim_a = num_actor_obs + body_dim
        mlp_input_dim_c = num_critic_obs + body_dim
    ...
    def update_distribution(self, observations, embodys = None):
        if embodys is None:
            embodys = torch.ones((observations.shape[0], self.body_dim)).to(observations.device)
    ...
    def evaluate(self, critic_observations, embodys = None, **kwargs):
        if embodys is None:
            embodys = torch.ones((critic_observations.shape[0], self.body_dim)).to(critic_observations.device)
